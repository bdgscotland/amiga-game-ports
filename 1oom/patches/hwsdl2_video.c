...
static void video_update(void)
{
    ...
    if (video.texture_upscaled == NULL) {
        ...
        t4 = SDL_GetTicks();
        if (vframe_count < 20) {
            SDL_Log("F%d: blit=%lu uptex=%lu copy=%lu present=%lu total=%lu",
                vframe_count, (unsigned long)(t1-t0), (unsigned long)(t2-t1),
                (unsigned long)(t3-t2), (unsigned long)(t4-t3), (unsigned long)(t4-t0));
        }
        ++vframe_count;
        return;
    }
    ...
    if (vframe_count < 20) {
        SDL_Log("F%d: blit=%lu uptex=%lu copy+upscale=%lu present=%lu total=%lu",
            vframe_count, (unsigned long)(t1-t0), (unsigned long)(t2-t1),
            (unsigned long)(t3-t2), (unsigned long)(t4-t3), (unsigned long)(t4-t0));
    }
    ++vframe_count;

...
Uint32 timer_callback(Uint32 interval, void *param) {
	...
	SDL_PushEvent(&event);
	if (cb_count < 5) {
		SDL_Log("POP: timer_callback fired #%d", cb_count);
	}
	++cb_count;
	return interval;
}
...
#else
	/* amiport: fix compat timer -- original code creates then immediately
	 * removes the timer. Remove OLD timer first, then create new. */
	if (global_timer != 0) {
		SDL_RemoveTimer(global_timer);
		global_timer = 0;
	}
	global_timer = SDL_AddTimer(1000/frequency, timer_callback, NULL);
	SDL_Log("POP: SDL_AddTimer(%d) = %d", 1000/frequency, (int)global_timer);
	if (global_timer == 0) {

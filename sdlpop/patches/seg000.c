...
void pop_main() {
	...
	SDL_Log("POP: load_global_options");
	load_global_options();
	SDL_Log("POP: check_mod_param");
	check_mod_param();
#ifdef USE_MENU
	...
	SDL_Log("POP: parse_grmode");
	/*video_mode =*/ parse_grmode();
	SDL_Log("POP: rect_sthg");
	current_target_surface = rect_sthg(onscreen_surface_, &screen_rect);
	SDL_Log("POP: set_hc_pal");
	set_hc_pal();
	SDL_Log("POP: init_copyprot_dialog");
	init_copyprot_dialog();

	SDL_Log("POP: load_mod_options");
	load_mod_options();
	...
	SDL_Log("POP: init_timer");
	init_timer(BASE_FPS);
	SDL_Log("POP: parse_cmdline_sound");
	parse_cmdline_sound();
